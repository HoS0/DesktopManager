...
QString QJsonView::variantToHtml(QVariant data)
{
	if (data.type() == QVariant::String || data.type() == QVariant::ByteArray)
		return "<span style=\"color: #006000\">\"" + QString(data.toString()).toHtmlEscaped() + "\"</span>";
	else if (data.type() == QVariant::Int || data.type() == QVariant::LongLong)
		return "<span style=\"color: #800000\">" + QString(data.toString()).toHtmlEscaped() + "</span>";
	else if (data.type() == QVariant::Double)
		return "<span style=\"color: #800080\">" + QString(data.toString()).toHtmlEscaped() + "</span>";
	...
	else if (data.type() == QVariant::Map)
	{
		...
		for (i = map.begin(); i != map.end(); ++i)
		{
			if (i != map.begin())
				str += "<span style=\"color: #606060\"><b>, </b></span>";
			str += QString(i.key()).toHtmlEscaped() + ": " + variantToHtml(i.value());
		}
		//entry "children"
		if (containsChildren) {
			if (!map.isEmpty())
	...
	if (isExpandable())
	{
		...
		else if (v.type() == QVariant::Map)
		{
			...
			for (i = map.begin(); i != map.end(); ++i)
			{
				...
				k->setText("<span style=\"font-family: monospace\">" + QString(i.key()).toHtmlEscaped() + ": </span>");
